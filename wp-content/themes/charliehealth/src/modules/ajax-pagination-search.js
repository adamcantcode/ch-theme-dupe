  ...
  const renderHTML = (post, html) => {
    ...
    if (tags) {
      html += `${tags
        .map(
          (tag) =>
            `<div class="absolute rounded-t-lg top-sp-4 left-sp-4"><a href="${window.location.origin}/resources/${tag.slug}" class="relative inline-block no-underline rounded-pill px-base5-3 py-base5-2 text-primary bg-white group-hover:bg-white-hover border border-white z-[6] text-h5-base">${tag.name}</a></div>`
        )
        .join('')}`;
    }
    html += `</div><!--end three-->`;
