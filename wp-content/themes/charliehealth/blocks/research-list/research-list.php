...
<div <?= $anchor ?: ''; ?>class="<?= $className; ?> relative grid lg:grid-cols-[1fr_2fr] lg:gap-sp-16 gap-sp-8">
  ...
  <div class="sticky-split-info">
    ...
    <?php if (have_rows('articles')) : while (have_rows('articles')) : the_row(); ?>
        ...
        ?>
        <a href="<?= $link['link']; ?>" target="<?= $link['target']; ?>" class="font-heading text-[20px] leading-[1.4] mb-0 inline-block font-normal no-underline text-primary py-sp-6"><?= $link['title']; ?> <svg width="17" height="14" viewBox="0 0 17 14" fill="none" xmlns="http://www.w3.org/2000/svg" class="inline-block align-[initial]">
