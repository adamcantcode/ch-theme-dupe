<div class="grid items-center lg:grid-cols-2 gap-base5-4">
  <div>
    <InnerBlocks />
  </div>
  <div>
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 674 583">
      <defs>
        <mask id="mask1">
          <path fill="#fff" d="M5.35275 358.167c0 127.537 72.68665 201.817 187.30725 201.817 86.665 0 138.384-42.045 138.384-114.924 0-99.506-79.675-138.748-234.833-142.953-34.9455 0-64.2997 0-90.85825 1.402v54.658Z"></path>
        </mask>
        <mask id="mask2">
          <path fill="#fff" d="M588.321 160.907C577.629 58.5896 517.037.292236 425.556.292236c-89.106 0-147.321 51.158864-154.45 160.614764H588.321Z"></path>
        </mask>
        <mask id="mask3">
          <circle cx="60.2677" cy="60.2677" r="60.2677" transform="matrix(-1 0 0 1 120.535 162.875)" fill="#fff"></circle>
        </mask>
        <mask id="mask4">
          <circle cx="60.2677" cy="60.2677" r="60.2677" transform="matrix(-1 0 0 1 228.654 461.495)" fill="#fff"></circle>
        </mask>
        <mask id="mask5">
          <path fill="#fff" d="M247.465 82.2639c-30.549 1.629-55.719 25.9201-58.544 56.5951-13.15 144.201 12.584 255.356 76.544 330.457 7.57 8.918 15.706 17.32 24.358 25.153l4.144 3.665c62.971 54.511 130.062 59.77 133.3 59.997l2.354.108 1.919-.156c5.121-.371 92.956-7.905 161.695-88.779 63.925-75.173 89.66-186.351 76.522-330.421-2.837-30.699-28.007-54.9901-58.521-56.6191-40.827-2.168-322.979-2.168-363.771 0Z"></path>
        </mask>
      </defs>

      <line y1="-0.5" x2="160.18" y2="-0.5" transform="matrix(-1 0 0 1 239.452 225.746)" stroke="#161A3D" />
      <rect x="0" y="250" width="350" height="350" fill="#FFFFFF" mask="url(#mask1)"></rect>
      <line y1="-0.5" x2="160.18" y2="-0.5" transform="matrix(-0.707107 0.707107 0.707107 0.707107 295.913 402.63)" stroke="#161A3D" />
      <rect x="250" y="0" width="350" height="350" fill="#FBF1D5" mask="url(#mask2)"></rect>
      <image href="https://placehold.co/500x500?text=3" alt="TERTIARY_IMAGE_ALT" class="tertiary" x="0" y="158" width="130" height="130" mask="url(#mask3)"></image>
      <image href="https://placehold.co/500x500?text=3.2" alt="TERTIARY_IMAGE_ALT" class="tertiary" x="0" y="158" width="130" height="130" mask="url(#mask3)"></image>
      <image href="https://placehold.co/500x500?text=3.3" alt="TERTIARY_IMAGE_ALT" class="tertiary" x="0" y="158" width="130" height="130" mask="url(#mask3)"></image>
      <image href="https://placehold.co/500x500?text=2" alt="SECONDARY_IMAGE_ALT" class="secondary" x="104" y="456" width="130" height="130" mask="url(#mask4)"></image>
      <image href="https://placehold.co/500x500?text=2.2" alt="SECONDARY_IMAGE_ALT" class="secondary" x="104" y="456" width="130" height="130" mask="url(#mask4)"></image>
      <image href="https://placehold.co/500x500?text=2.3" alt="SECONDARY_IMAGE_ALT" class="secondary" x="104" y="456" width="130" height="130" mask="url(#mask4)"></image>
      <image href="https://placehold.co/500x500?text=1" alt="MAIN_IMAGE_ALT" class="main" x="180" y="70" width="500" height="500" mask="url(#mask5)"></image>
      <image href="https://placehold.co/500x500?text=1.2" alt="MAIN_IMAGE_ALT" class="main" x="180" y="70" width="500" height="500" mask="url(#mask5)"></image>
      <image href="https://placehold.co/500x500?text=1.3" alt="MAIN_IMAGE_ALT" class="main" x="180" y="70" width="500" height="500" mask="url(#mask5)"></image>
    </svg>
  </div>
</div>