...
<script>
  window.addEventListener('DOMContentLoaded', () => {
    ...
    function renderSchedule(events) {
      ...
      Object.entries(grouped).forEach(([day, times]) => {
        ...
        Object.entries(times).forEach(([time, evs]) => {
          ...
          timeBlock.className = "mb-base5-10 event-day-js";
          timeBlock.innerHTML = `
          <h4 class="bg-[#fbd4c9] text-[#7c2d12] px-4 py-1 rounded-md inline-block">${time}</h4>
          <div class="space-y-6">
            ${evs.map(e => `
              <div class="border rounded-md p-base5-5 border-primary event-cards-js">
      ...
      document.dispatchEvent(new Event('scheduleRendered'));
    }
    ...
    [dayFilter, timeFilter].forEach(select => {
    ...
    tagFilters.addEventListener('click', e => {
      if (e.target.matches('.tag-badge')) {
        ...
        document.dispatchEvent(new CustomEvent('tagFilterToggled', {
          detail: e.target
        }));
      }
    });
