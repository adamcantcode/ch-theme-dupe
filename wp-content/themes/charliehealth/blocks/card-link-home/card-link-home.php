...
<div class="grid items-center grid-cols-1 lg:grid-cols-2 gap-base5-4">
  <div>
    <div class="rounded-[16px] grid lg:grid-cols-[1fr_2fr] grid-cols-1 lg:py-sp-12 py-sp-8 px-sp-8 gap-sp-8 items-center relative group <?= $bg; ?>">
      ...
      <div>
        <h3><a href="<?= $link['url']; ?>" target="<?= $link['target']; ?>" class="stretched-link"><?= $cardHeading; ?><svg width="17" height="14" viewBox="0 0 17 14" fill="<?php isset($block['gradient']) ? '#161A3D' : 'white'; ?>" xmlns="http://www.w3.org/2000/svg" class="inline-block transition-all duration-300 middle ml-sp-2 group-hover:translate-x-sp-2">
