  ...
  <div class="container">
    <div class="grid grid-cols-1 lg:grid-cols-[3fr_9fr] gap-sp-5">
      ...
      <div class="">
        ...
        if ($query->have_posts()) : while ($query->have_posts()) : $query->the_post();
            $link = get_field('link') ?: get_the_permalink();
            $target = get_field('link') ? '_blank' : '_self';
        ?>
            <a href="<?= $link; ?>" target="<?= $target; ?>" class="grid no-underline border-b opacity-0 first:border-t py-sp-6 border-primary group press-release-js noshow not-loaded">
